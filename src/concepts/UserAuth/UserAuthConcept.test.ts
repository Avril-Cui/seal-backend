...
Deno.test("Principle: User accounts are unique by email, and users can log in", async (t) => {
  ...
  try {
    await t.step("trace: Alice signs up for an account", async () => {
      ...
      console.log(
        `Trace: Signup successful. User ID: ${(signupResult as any).user._id}`,
      );
    });
    ...
    await t.step(
      ...
      async () => {
        ...
        console.log(
          `Trace: Login successful for user: ${
            (loginResult as any).user.email
          }`,
        );
      },
  ...
  try {
    await t.step(
      ...
      async () => {
        ...
        assertEquals(
          "error" in initialSignup,
          false,
          "Initial signup must succeed.",
        );
        assertExists(
          (initialSignup as any).user,
        ...
        console.log(
          `Test: Attempting signup again for ${TEST_EMAIL_1} (should fail).`,
        );
        ...
        assertEquals(
          "error" in duplicateSignup,
          true,
          "Duplicate email signup must return an error.",
        );
        assertEquals(
    ...
    await t.step(
      "effects: creates new user, adds to RegisteredUsers, returns user",
      async () => {
        ...
        assertEquals(
          "error" in newSignupResult,
          false,
          "New user signup must succeed.",
        );
        ...
        assertEquals(
          newUser.email,
          TEST_EMAIL_2,
          "The created user's email should match input.",
        );
        assertEquals(
          newUser.password,
          TEST_PASSWORD_2,
          "The created user's password should match input.",
        );
        console.log(
        ...
        assertEquals(
          "error" in loginVerification,
          false,
          "Login for newly signed up user must succeed.",
        );
        assertEquals(
        ...
        console.log(
          `Test: User ${TEST_EMAIL_2} successfully logged in, confirming creation effect.`,
        );
      },
    );
  } finally {
...
Deno.test("Action: login - requirements and effects", async (t) => {
  ...
  try {
    await t.step(
      "requires: user exists with matching (email, password) - non-existent email",
      async () => {
        const nonExistentEmail = "nonexistent@example.com";
        console.log(
          `Test: Attempting login with non-existent email: ${nonExistentEmail}.`,
        );
        const loginResult = await userAuthConcept.login({
    ...
    await t.step(
      "requires: user exists with matching (email, password) - wrong password",
      async () => {
        console.log(
    ...
    await t.step(
      "effects: return this user upon successful login",
      async () => {
        console.log(
        ...
        console.log(
          `Test: Successful login returned user: ${loggedInUser.email}.`,
        );
      },
    );
  } finally {
