...
export interface AmazonAPIClient {
  fetchItemDetails(url: string): Promise<
    | {
        itemName: string;
        description: string;
        photo: string;
...
export class RealAmazonAPIClient implements AmazonAPIClient {
  async fetchItemDetails(url: string): Promise<
    | {
        itemName: string;
        description: string;
        photo: string;
  ...
  > {
    ...
    try {
      ...
      if (priceWhole) {
        const wholeText =
          priceWhole.textContent?.trim().replace(/[,\.]/g, "") || "0";
      ...
      if (price === 0) {
        ...
        if (priceSpan) {
          const priceText =
            priceSpan.textContent?.trim().replace(/[^0-9.]/g, "") || "0";
      ...
      if (!photo) {
        photo =
          ...
          encodeURIComponent(itemName.slice(0, 20));
      }
...
/**
 * Custom implementation of AmazonAPIClient for development/testing.
 * Falls back to this if real scraping fails.
 */
export class CustomAmazonAPIClient implements AmazonAPIClient {
  ...
  > {
    ...
    if (!("error" in result)) {
      const hasValidTitle =
        result.itemName && result.itemName !== "Amazon Product";
      const hasValidPrice = result.price > 0;
      const hasValidImage =
        result.photo && !result.photo.includes("placeholder");

      // If we got at least 2 meaningful pieces of data, use the scraped result
      if (
        (hasValidTitle && hasValidPrice) ||
        (hasValidTitle && hasValidImage) ||
        (hasValidPrice && hasValidImage)
      ) {
        console.log(
          `Using scraped data: ${result.itemName} - $${result.price}`
        );
        return result;
      }
    ...
    console.log(
      `CustomAmazonAPIClient: Falling back to mock data for URL: ${url}`
    );
    ...
    if (
      url.includes("book") ||
      url.includes("Babs") ||
      url.includes("Holiday") ||
      url.includes("Recipes")
    ) {
      itemName = "Celebrate with Babs: Holiday Recipes & Traditions";
      description =
        "A heartwarming cookbook filled with cherished holiday recipes, family traditions, and festive celebrations from beloved TikTok grandmother Babs.";
      price = 19.99;
      ...
      description =
        "Kids tablet with parental controls and educational content. Includes a protective case.";
      price = 119.99;
