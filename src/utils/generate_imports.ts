...
async function discoverConcepts(baseDir: string): Promise<ConceptInfo[]> {
  ...
  for await (const dirEntry of Deno.readDir(absoluteBaseDir)) {
    ...
    try {
      ...
      const conceptName =
        conceptDirName.charAt(0).toUpperCase() + conceptDirName.slice(1);
...
function generateBarrelFileContent(
  ...
  const header = `// This file is auto-generated by the \`generate_imports.ts\` script.
...
`;
  ...
  const instantiations = concepts
    .map((c) => {
      ...
      if (c.name === "ItemCollection") {
        return `export const ${c.name} = Engine.instrumentConcept(new ${c.name}Concept(db, amazonAPI, geminiLLM));`;
      }
...
async function discoverSyncs(baseDir: string): Promise<SyncInfo[]> {
  ...
  try {
    for await (const entry of walk(absoluteBaseDir, {
      includeFiles: true,
      includeDirs: false,
      exts: [".sync.ts"],
    })) {
