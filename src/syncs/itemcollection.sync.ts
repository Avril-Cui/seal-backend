...
export const GetUserWishListRequest: Sync = ({
  ...
  where: async (frames) => {
    ...
    const hasCompletedQueueValue =
      !("error" in queueResult[0]) && queueResult[0].completedQueue >= 10;
    ...
    if ("error" in queueResult[0]) {
      ...
      console.log(
        `[GetUserWishListRequest] Completed queue count: ${queueResult[0].completedQueue}`
      );
    }
    ...
    const collectedItems =
      itemsArray.length > 0 && itemsArray[0][items]
        ...
        itemsList.map(async (itemDoc: any) => {
          ...
          try {
            ...
            if (!("error" in statsResult[0])) {
              ...
              console.log(
                `[GetUserWishListRequest] No stats for item ${itemDoc._id}: ${statsResult[0].error}`
              );
            }
          } catch (error) {
            console.warn(
